...
class Ball:
    def __init__(self, canvas, color, winW, winH, racket, touch):
        self.canvas = canvas
        self.racket = racket
    ...
    def ballMove(self):
        ...
        if self.hitRacket(ballPos) == True:     # 偵測是否撞到球拍
            self.touch += 1
            print('碰到', self.touch, '次')
            self.y = -step
        if ballPos[3] >= winH:                  # 如果球接觸到畫布底端
            if input('play or not?(y/n)') == 'y':
                self.notTouchBottom = True
            else:
                self.notTouchBottom = False
class Racket:                                                       
    def __init__(self, canvas, color, touch):
        self.canvas = canvas
...
racket = Racket(canvas, 'purple', touch)               # 定義紫色球拍
ball = Ball(canvas,'yellow',winW,winH,racket,touch)   # 定義球物件
